...
- Added the Fitting Report `Include Weld` option. Fitting Report PDF and Excel output now group by resolved package instead of material grade, place `NO PACKAGE ASSIGNED` last, and use `Description BOM` for equal-size tees when the parameter is populated.
